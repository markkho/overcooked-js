...
class TestGridworld(unittest.TestCase):
    def setUp(self):
        self.mdp1 = OvercookedGridworld.from_file('layouts/mdp_test.layout')
    ...
    def test_start_positions(self):
        expected_state = OvercookedState(
            [PlayerState((1, 2), Direction.NORTH),
             PlayerState((3, 1), Direction.NORTH)], {}, order_list=['any'])
        self.assertEqual(self.mdp1.get_start_state(['any']), expected_state)

    def test_file_constructor(self):
        mdp = OvercookedGridworld.from_file('layouts/corridor.layout')
        self.assertEqual(mdp.get_start_state(['onion']), OvercookedState(
            [PlayerState((3, 1), Direction.NORTH),
             PlayerState((10, 1), Direction.NORTH)], {}, order_list=['onion']))
    ...
    def test_actions(self):
        bad_state = OvercookedState(
            [PlayerState((0, 0), Direction.NORTH),
             PlayerState((3, 1), Direction.NORTH)], {}, order_list=['any'])
        with self.assertRaises(AssertionError):
            self.mdp1.get_actions(bad_state)

        self.assertEqual(
            self.mdp1.get_actions(self.mdp1.get_start_state(['any'])),
    ...
    def test_transitions_and_environment(self):
        ...
        with self.assertRaises(AssertionError):
            self.mdp1.get_transition_states_and_probs(bad_state, stay)
        ...
        def check_transition(action, expected_state, expected_reward=0):
            ...
            ((pred_state,
              prob),), reward = self.mdp1.get_transition_states_and_probs(
                state, action)
        ...
        check_transition([stay, stay], OvercookedState(
            [P((2, 1), n, Obj('dish', (2, 1))),
             P((3, 1), e, Obj('onion', (3, 1)))],
            {(2, 0): Obj('soup', (2, 0), ('onion', 3, 6))},
            order_list=['onion', 'any']))

        check_transition([interact, interact], OvercookedState(
            [P((2, 1), n, Obj('soup', (2, 1), ('onion', 3, 6))),
             P((3, 1), e, Obj('onion', (3, 1)))],
            {}, order_list=['onion', 'any']))


        check_transition([e, w], OvercookedState(
            [P((2, 1), e, Obj('soup', (2, 1), ('onion', 3, 6))),
             P((3, 1), w, Obj('onion', (3, 1)))],
            {}, order_list=['onion', 'any']))

        check_transition([e, s], OvercookedState(
            [P((3, 1), e, Obj('soup', (3, 1), ('onion', 3, 6))),
             P((3, 2), s, Obj('onion', (3, 2)))],
            {}, order_list=['onion', 'any']))

        check_transition([s, interact], OvercookedState(
            [P((3, 1), s, Obj('soup', (3, 1), ('onion', 3, 6))),
             P((3, 2), s, Obj('onion', (3, 2)))],
            {}, order_list=['onion', 'any']))

        check_transition([s, w], OvercookedState(
            [P((3, 2), s, Obj('soup', (3, 2), ('onion', 3, 6))),
